...
export default function Dashboard() {
  ...
  const handleToggleComments = async (postId) => {
    if (openCommentPostId === postId) {
      ...
      await dispatch(getAllComments({ post_id: postId }));
      setOpenCommentPostId(postId); // open + load comments
    }
  };
                    ...
                    <div>
                      <div style={{ display: "flex", gap: "1.2rem", justifyContent: "space-between" }}>
                        ...
                        {post?.userId?._id === authState?.user?.userId?._id && (
                          ...
                          >
                            <svg style={{ height: "1.4em", color: "red" }}
                        ...
                        <div className={styles.commentsSection}>
                          ...
                          <p className={styles.commentCount}>
                            ...
                            {postState.comments.length === 0
                              ? "No comments"
                              : `${postState.comments.length} comment${postState.comments.length !== 1 ? "s" : ""}`}
                          </p>
                          ...
                          {postState.comments.map((comment) => {
                            // Check if current logged-in user is the comment author
                              ...
                              <div key={comment._id} className={styles.singleComment}>
                                ...
                                <div className={styles.commentMeta}>
                                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                    ...
                                    {comment.userId?._id === authState?.user?.userId?._id && (
  <button
    onClick={() => handleDeleteComment(comment._id, post._id)}  // ✅ both args passed
